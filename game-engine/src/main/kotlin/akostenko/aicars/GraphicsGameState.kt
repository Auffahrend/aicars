...
abstract class GraphicsGameState : BasicGameState() {
    ...
    protected lateinit var cameraOffset: Cartesian
     ...
     */
    protected fun drawTrackMarker(g: Graphics, marker: TrackMarker, camera: Cartesian, scale: Scale, color: Color, size: Float) {

        var text = marker.text.trim()
        if (text.length > 5) text = text.substring(0..4)

        //1.5 is character's height/width
        val textOffset = Cartesian(-text.length.toFloat() / 2 * size / 1.5, -size / 2.0)
        g.color = color
        ...
        g.drawString(marker.text,
                scale.to(marker.position.asCartesian().x - camera.x + textOffset.x) + cameraOffset.x.toFloat(),
                scale.to(marker.position.asCartesian().y - camera.y + textOffset.y) + cameraOffset.y.toFloat())
    }

    protected fun drawRealLine(g: Graphics, line: Line, camera: Cartesian, scale: Scale, color: Color, width: Float) {
    ...
    private fun drawStraightLine(g: Graphics, line: StraightLine, camera: Cartesian, scale: Scale, color: Color, width: Float) {
        g.lineWidth = width
        g.color = color
        g.drawLine(
    ...
    private fun drawArc(g: Graphics, arcLine: ArcLine, camera: Cartesian, scale: Scale, color: Color, width: Float) {
        g.lineWidth = width
        g.color = color
